...
namespace tinytorch
{
    ...
    void backward(Tensor loss)
    {
        ...
        while (!node_stack.empty())
        {
            ...
            std::vector<Tensor> next_gradients = (*current_node).backward(grad_map[current_node]);
            ...
            for (size_t i = 0; i < (*current_node).next.size(); i++)
            {
                ...
                if (next)
                {
                    ...
                    grad_map[next_node][next->identifier].addInplace(next_gradients[next->identifier]);
