...
struct TensorImpl {
 ...
 public:
  ...
  // constructors
  TensorImpl(size_t size, std::vector<size_t> shape, std::string arch)
      : data_(size), ndim_(shape.size()), shape_(shape), stride_(ndim_), arch_(arch) {
    init_stride();
  }
  TensorImpl(std::vector<data_t> data, std::vector<size_t> shape, std::string arch)
      : data_(data), ndim_(shape.size()), shape_(shape), stride_(ndim_), arch_(arch) {
    init_stride();
  }
  TensorImpl(std::vector<data_t> data, std::vector<size_t> shape,
             std::vector<size_t> stride)
      : data_(data), ndim_(shape.size()), shape_(shape), stride_(stride) {}
...
struct Tensor {
 ...
 public:
  ...
  Tensor(size_t size = 0, std::string arch = "host") {
    ...
    impl_ = std::make_shared<TensorImpl>(size, shape, arch);
  }
  Tensor(std::vector<data_t> data, std::string arch = "host") {
    std::vector<size_t> shape = {data.size()};
    impl_ = std::make_shared<TensorImpl>(data, shape, arch);
  }
  Tensor(std::shared_ptr<TensorImpl> impl) : impl_(impl) { }

  // operator override
  data_t& operator[](size_t idx) { return impl_->operator[]({idx}); }
  data_t& operator[](std::vector<size_t> idxs) { return impl_->operator[](idxs); }
