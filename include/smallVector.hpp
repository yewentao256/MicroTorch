...
class SmallVectorTemplateCommon
 ...
 protected:
  ...
  template <class IterType, std::enable_if_t<
                            !std::is_same<std::remove_const_t<IterType>, T*>::value,
                            bool> = false>
 ...
 public:
  ...
  template <typename IterType,
            typename = std::enable_if_t<std::is_convertible_v<
                ...
                std::input_iterator_tag>>>
  iterator insert(iterator I, IterType From, IterType To) {
    // Convert iterator to elt# to avoid invalidating iterator when we reserve()
    size_t InsertElt = I - this->begin();
...
template <typename T>
SmallVectorImpl<T>&
SmallVectorImpl<T>::operator=(SmallVectorImpl<T>&& RHS) noexcept(
    std::is_nothrow_move_constructible_v<T> &&
    std::is_nothrow_destructible_v<T>) {
  // Avoid self-assignment.
  if (this == &RHS) return *this;
...
// the pointer math in SmallVectorTemplateCommon::getFirstEl() is
// well-defined.
template <typename T>
...
// elements N, it is recommended to use SmallVector<T> (that is,
// omitting the N). This will choose a default number of inlined elements
// reasonable for allocation on the stack (for example, trying to keep
...
template <typename T>
inline void swap(microtorch::SmallVectorImpl<T>& LHS,
                 microtorch::SmallVectorImpl<T>& RHS) {
  LHS.swap(RHS);
}

// Implement std::swap in terms of SmallVector swap.
template <typename T, unsigned N>
inline void swap(microtorch::SmallVector<T, N>& LHS,
                 microtorch::SmallVector<T, N>& RHS) {
  LHS.swap(RHS);
}
