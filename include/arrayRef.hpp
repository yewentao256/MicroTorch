...
class MArrayRef final {
 ...
 public:
  ...
  /* implicit */ constexpr MArrayRef() : Data(nullptr), Length(0) {}

  /// Construct an MArrayRef from a single element.
  // TODO Make this explicit
  constexpr MArrayRef(const T& OneElt) : Data(&OneElt), Length(1) {}

  /// Construct an MArrayRef from a pointer and length.
  MArrayRef(const T* data, size_t length)
      : Data(data), Length(length) {
    debugCheckNullptrInvariant();
  }

  /// Construct an MArrayRef from a range.
  MArrayRef(const T* begin, const T* end)
  ...
  template <typename U>
  /* implicit */ MArrayRef(const SmallVectorTemplateCommon<T, U>& Vec)
  ...
  template <
      ...
      typename = std::enable_if_t<std::is_same<
          ...
          T*>::value>>
  /* implicit */ MArrayRef(const Container& container)
  ...
  // bitfield.
  template <typename A>
  /* implicit */ MArrayRef(const std::vector<T, A>& Vec)
      : Data(Vec.data()), Length(Vec.size()) {
    static_assert(
        !std::is_same<T, bool>::value,
        "MArrayRef<bool> cannot be constructed from a std::vector<bool> bitfield.");
  }

  /// Construct an MArrayRef from a std::array
  template <size_t N>
  /* implicit */ constexpr MArrayRef(const std::array<T, N>& Arr)
      : Data(Arr.data()), Length(N) {}

  /// Construct an MArrayRef from a C array.
  template <size_t N>
  /* implicit */ constexpr MArrayRef(const T (&Arr)[N]) : Data(Arr), Length(N) {}

  /// Construct an MArrayRef from a std::initializer_list.
  /* implicit */ constexpr MArrayRef(const std::initializer_list<T>& Vec)
      : Data(
  ...
  const T& front() const {
    TORCH_CHECK(
        !empty(), "MArrayRef: attempted to access front() of empty list");
    return Data[0];
  }

  /// back - Get the last element.
  const T& back() const {
    TORCH_CHECK(!empty(), "MArrayRef: attempted to access back() of empty list");
  ...
  MArrayRef<T> slice(size_t N, size_t M)
      const {
    TORCH_CHECK(
        N + M <= size(),
        "MArrayRef: invalid slice, N = ",
        N,
        "; M = ",
        M,
        "; size = ",
  ...
  MArrayRef<T> slice(size_t N) const {
    TORCH_CHECK(
        N <= size(), "MArrayRef: invalid slice, N = ", N, "; size = ", size());
...
/// @name MArrayRef Convenience constructors
/// @{
