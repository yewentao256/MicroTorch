...
void export_tensor_class(py::module &m) {
  py::class_<Tensor>(m, "Tensor")
      ...
      .def("__repr__",
           [](Tensor &t) {
             return "<tinytorch.Tensor [size=" + std::to_string(t.size()) +
                    "]>";
           })
      .def(
...
void export_tensor_function(py::module &m) {
  m.def("zero", &tinytorch::zero, "initialize a tensor with all zero", py::arg("size"))
  .def("ones", &tinytorch::ones, "initialize a tensor with all ones", py::arg("size"))
  .def("rand", &tinytorch::rand, "initialize a tensor with random numbers", py::arg("size"))
